...
import rage.pitclient.auth.Auth;
...
public class Main
{
    public static void main(String[] p_main_0_)
    {
        ...
        OptionSpec<String> username = optionparser.accepts("username").withRequiredArg().defaultsTo("Player" + Minecraft.getSystemTime() % 1000L, new String[0]);
        OptionSpec<String> uuid = optionparser.accepts("uuid").withRequiredArg();
        OptionSpec<String> token = optionparser.accepts("accessToken").withRequiredArg().required();
        ...
        OptionSpec<String> pass = optionparser.accepts("password").withRequiredArg();
        ...
        String s4 = optionset.has(uuid) ? (String)uuid.value(optionset) : (String)username.value(optionset);
        ...
        Session session = new Session((String)username.value(optionset), s4, (String)token.value(optionset), (String)optionspec18.value(optionset));
        
        String[] sess = Yggdrasil.login(username.value(optionset), pass.value(optionset));
        if (sess != null && sess.length > 1) {
        	propertymap = (PropertyMap)gson.fromJson(sess[3], PropertyMap.class);
        	session = new Session(sess[0], sess[1], sess[2], optionspec18.value(optionset));
        }
